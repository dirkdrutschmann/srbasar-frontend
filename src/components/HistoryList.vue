        ...
        <div class="row mx-2">
          <div v-if="refInclude(props.row.sr1)" class="col-lg-6 mt-1">
            <SRList 
              :key="getId" 
              :sr="props.row.sr1" 
              :besetzt="props.row.sr1Besetzt" 
              :info="props.row.sr1Info"
              :basar="props.row.sr1Basar"
              :bonus="props.row.sr1Bonus" 
              position="1" 
              :match="props.row.matchId" 
              :name="props.row.sr1Name" 
              :email="props.row.sr1Mail"
              @update="(payload) => updateRef(payload)"
            />
          </div>
          <div v-else class="col-lg-6 mt-1">
            <div class="border border-white p-2 rounded-3"
                 :class="{'offen': props.row.sr1Basar && !props.row.sr1Besetzt}">
              <p>SR1:</p>
          ...
          <div v-if="refInclude(props.row.sr2)" class="col-lg-6 mt-1">
            <SRList 
              :key="getId" 
              :sr="props.row.sr2" 
              :besetzt="props.row.sr2Besetzt" 
              :info="props.row.sr2Info"
              :basar="props.row.sr2Basar"
              :bonus="props.row.sr2Bonus" 
              position="2" 
              :match="props.row.matchId" 
              :name="props.row.sr2Name" 
              :email="props.row.sr2Mail"
              @update="(payload) => updateRef(payload)"
            />
          </div>
          <div v-else class="col-lg-6 mt-1">
            <div class="border border-white p-2 rounded-3"
                 :class="{'offen': props.row.sr2Basar && !props.row.sr2Besetzt}">
              <p>SR2:</p>
...
const updateRef = (payload) => {
  var game = games.value.findIndex(_g => _g.matchId === payload.match)
  console.log(game, games.value[game])
  try {
...
onMounted(async () => {
  ...
  games.value = spiele.filter((game) => {
    return (Math.floor(new Date() / 1000) - Math.floor(new Date(game.kickoffDate) / 1000)) > 0
  })
  ...
  totalGames.value = spiele.map((game) => {
    return {
      ...
      SR1Name: refInclude(game.sr1Name) ? game.sr1Name : "",
      ...
      SR2Info: refInclude(game.sr1) ? game.sr2Info : "",
      SR2Name: refInclude(game.sr2Name) ? game.sr2Name : "",
    }
  })
})
</script>
