...
test('uses the shared desktop card height and opens a detail modal before requesting', async () => {
  ...
  assert.doesNotMatch(detail, /paymentFormulaLabel/)
  assert.doesNotMatch(detail, /10 € je Turnierstunde/)
  assert.match(detail, /niedrigschwelliges Angebot/)
  assert.match(detail, /Unterstützung durch engagierte Schiedsrichter/)
